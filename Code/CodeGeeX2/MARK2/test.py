...
def model_inference():
    ...
    with open(src_filename, 'r') as f1,open(trg_filename, 'r') as f2:
        ...
        for line1,line2 in zip(f1,f2):
            ...
            if input_ids.size(1) >= args.max_source_length:
                ...
                try:
                    generated_ids = model.generate(input_ids=input_ids, max_new_tokens=args.max_target_length, num_beams=args.beam_size, num_return_sequences=args.output_size, early_stopping=True,
                ...
                logger.info(" %d/%d, Pre_EM = %s, Pre_Fix = %s", i, data_size, str(EM_result), str(pre_out)) 
    ...
    with codecs.open(args.output_dir+'/pre_file.txt', 'w', encoding='utf-8') as pre_file, codecs.open(args.output_dir+'/tgt_file.txt', 'w', encoding='utf-8') as tgt_file:
