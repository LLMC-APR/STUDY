...
def convert_examples_to_features(examples, tokenizer, args,stage=None):
    ...
    for example_index, example in enumerate(tqdm(examples,total=len(examples))):
        ##extract data flow
        code_tokens,dfg=extract_dataflow(example.source,parsers['c'],'c')
...
def main():
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval and (int(epoch) % 5 == 0 or int(epoch) > int(args.num_train_epochs) - 10):
                ...
                if dev_bleu > best_bleu:
                    ...
                    logger.info("  No Best BLEU, Now BLEU:%s",dev_bleu)
                    logger.info("  "+"*"*20)
    ...
    if args.do_test:
        ...
        for idx,file in enumerate(files):
            test_file_type = args.load_model_path
            checkpoint_type = test_file_type.split('/')[-2]   
            ...
            for batch in tqdm(eval_dataloader,total=len(eval_dataloader)):
                ...
                with torch.no_grad():
                    ...
                    for pred in preds:
                        beam_outs = []
                        for beam_pred in pred:
                            beam_pred = beam_pred.cpu().numpy()
                            beam_pred = list(beam_pred)
                            if 0 in beam_pred:
                                beam_pred = beam_pred[:beam_pred.index(0)]
                            text = tokenizer.decode(beam_pred, clean_up_tokenization_spaces=False)
                            beam_outs.append(text)
                        p.append(beam_outs)
            ...
            accs=[]
            good, bad = 0, 0
            with open(os.path.join(args.output_dir,"test_{}.output".format(str(checkpoint_type))),'w') as f, open(os.path.join(args.output_dir,"test_{}.gold".format(str(checkpoint_type))),'w') as f1:
                for ref,gold in zip(p,eval_examples):
                    for one_ref in ref:
                        if gold.target.strip() == one_ref.strip():
                            good += 1
                            break
            ...
            # dev_bleu=round(_bleu(os.path.join(args.output_dir, "test_{}.gold".format(str(checkpoint_type))).format(file), 
            #                      os.path.join(args.output_dir, "test_{}.output".format(str(checkpoint_type))).format(file)),2)
            logger.info("  %s = %s "%("good fixes",str(good)))
            logger.info("  %s = %s "%("bad fixes",str(bad)))
            logger.info("  %s = %s "%("all example",str(good+bad)))
            logger.info("  %s = %s "%("xMatch",str(result)))
