...
def test(args, model, tokenizer, device, epoch=0):
        ...
        with torch.no_grad():
                ...
                for pred in preds:
                    ...
                    for beam_pred in pred:
                        ...
                        try:
                            text = ids_to_strs(beam_pred, tokenizer)
                            beam_outs.append(text)
                        except:
                            beam_outs.append('<FIXS> Warning: This is a fault fixes <FIXE>')
    ...
    accs = []
    good, bad = 0, 0
    with open(os.path.join(args.output_dir,"test_{}.output".format(args.checkpoint_model_type)),'w') as f, open(os.path.join(args.output_dir,"test_{}.gold".format(args.checkpoint_model_type)),'w') as f1:
        for ref,gold in zip(p,eval_examples):
            for one_ref in ref:  
    ...
    # (goldMap, predictionMap) = bleu.computeMaps(predictions, os.path.join(args.output_dir, "test_{}.gold".format(args.checkpoint_model_type))) 
    # dev_bleu=round(bleu.bleuFromMaps(goldMap, predictionMap)[0],2)
...
def main():
    ...
    parser.add_argument("--checkpoint_model_type", default=None, type=str, 
                        help="load fine-tuning model for downstream task.")
    ...
    parser.add_argument("--warmup_steps", default=100, type=int,
                        help="Linear warmup over warmup_steps. default_value=0")
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval and (int(epoch)%1 == 0 or int(epoch) > int(args.num_train_epochs)-5):
